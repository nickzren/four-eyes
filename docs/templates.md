...
Do not read current-round other-reviewer output or orchestrator synthesis before writing your own review.
...
For manual Reviewer 2, prior work or context, including historical peer output or synthesis, does not by itself make the session ineligible or require `could-not-review`. Disclose relevant provenance in `Agent/session`.
Verify the exact artifact against canonical sources and evidence. Do not treat your own prior proposals or conclusions as evidence.
Do not read current-round other-reviewer output or orchestrator synthesis before writing your own review. Current-round exposure requires `Review status: could-not-review` and `Verdict: not issued`.
Do not paste secrets, raw credentials, token values, sensitive resource names, or raw plan output into the coordination record.
